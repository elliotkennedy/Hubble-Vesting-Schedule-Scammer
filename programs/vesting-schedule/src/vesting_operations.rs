...
use crate::{
    tokenoperation, utils, ClaimEffects, ErrorCode, VestingData, VestingSchedule, VestingUser,
};
...
pub fn initialize_vesting_data(
    vesting_data: &mut VestingData,
    vesting_vault_pubkey: Pubkey,
    pda_vesting_vault: VestingPdaAddress,
    admin: Pubkey,
    ...
    vesting_data.admin = admin;
    vesting_data.vesting_schedule = vesting_schedule;
    vesting_data.hbb_mint = hbb_mint;
}
...
pub fn claim<'info>(
    ...
    index: u64,
    vesting_data: &mut VestingData,
) -> Result<ClaimEffects, crate::ErrorCode> {
    if schedule.data[index as usize].status != utils::utils::EventStatus::PendingToken as u8 {
        return Ok(ClaimEffects {
            claimable_amount: 0,
        });
    }
    ...
    Ok(ClaimEffects {
        claimable_amount: can_claim_now,
    })
}
...
mod tests {
    ...
    pub fn test_add_user() -> Result<(), ErrorCode> {
        ...
        assert_eq!(
            Pubkey::from_str("29GPMU5gtBDbd368EwquqTmo33tKgvneAK9REmmxkqm8").unwrap(),
            vesting_schedule.borrow_mut().data[0].user
        );
        assert_eq!(
            Pubkey::from_str("8v1DhJaewvhbhDmptNrkYig7YFcExsRKteR3cYjLw2iy").unwrap(),
            vesting_schedule.borrow_mut().data[1].user
        );
        assert_eq!(
            utils::utils::EventStatus::PendingToken as u8,
            vesting_schedule.borrow_mut().data[0].status
        );
        assert_eq!(
            utils::utils::EventStatus::PendingToken as u8,
            vesting_schedule.borrow_mut().data[1].status
        );
        ...
        assert_eq!(
            utils::utils::EventStatus::Inactive as u8,
            vesting_schedule.borrow_mut().data[1].status
        );
        Ok(())
    }
