...
mod tests {
    ...
    fn test_generate_seed() {
        ...
        let pdapubkey =
            make_vesting_pda_pubkey(&owner.clone(), utils::utils::TOKEN_MINT_TAG, &program_id);
        println!("pubkey {:?}", &pdapubkey.key);
        println!("pubkey {:?}", &pdapubkey.seed);
        assert_eq!(
            Pubkey::from_str("6yUirNTpsj1jfpfaT1pyBcc772mYC2fwRSpKZ52pS6CM").unwrap(),
            pdapubkey.key
        );
    }
    #[test]
    fn test_create_borrow_account() {
        ...
        let program_id = Pubkey::from_str("7SeC6f66GuxEEE1PHmAabu1SYbLnayJkWNE6127BNUYc").unwrap();
        let (pda, _bump_seed) =
            Pubkey::find_program_address(&[b"vesting_schedule_seed"], &program_id);
        let x = Pubkey::create_with_seed(&owner, "vesting_schedule_seed", &program_id);
        println!("key {:?}", pda);
        assert_eq!(
            Pubkey::from_str("72DY6RSrCjYXEgPt2Yaz5jm3CdUDVrsemgtzxcEUj2fj").unwrap(),
            x.unwrap()
        );
    }
}
